...
class MainWin(Ui_MainWindow):
    def setupWin(self, MainWindow, datadict):
        ...
        self.settingsButton.clicked.connect(
            lambda: self.settingsButtonClicked(self.datadict)
        )
            ...
            if not (name in self.runningVM.keys()):
                ...
                if "RunVM" in self.datadict.keys():
                    ...
                    if "86BoxPath" in self.datadict.keys():
                        ...
                        if "LogEnable" in self.datadict.keys():
                            if self.datadict["LogEnable"]:
                                ...
                                log_path = os.path.join(
                                    self.datadict["LogPath"], name + ".log"
                                )
                                ops.append(log_path)
                        ops.append("-P")
                        ops.append(path)
                        ...
                        self.runningVM[name] = {
                            "process": subprocess.Popen(ops, env=new_env),
                            "server": server,
                        }

    def socketConnect(self, name):
        print("Socket connected:" + name)
        self.runningVM[name]["client"] = self.runningVM[name][
            "server"
        ].nextPendingConnection()
        print(self.runningVM[name]["client"])
        self.runningVM[name]["client"].disconnected.connect(
            self.runningVM[name]["client"].deleteLater
        )
    ...
    def addButtonfunc(self, datadict):
        if not (os.path.exists(datadict["86BoxPath"])):
            self.errorBox(
                self.window,
                "Invalid 86Box path",
                "Please review the settings and define a 86Box path",
            )
            return
        elif not (os.path.exists(datadict["VMPath"])):
            self.errorBox(
                self.window,
                "Invalid VM storage path",
                "Please review the settings and define a path to VM storage",
            )
            return
            ...
            if not (os.path.exists(datadict["RomPath"])):
                self.errorBox(
                    self.window,
                    "Invalid Rom storage path",
                    "Please review the settings and define a path to Rom Storage",
                )
                return
